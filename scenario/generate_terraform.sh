...
# === 4.b Asociación automática de IPs flotantes a las instancias ===
echo "=== 4.b Asociación automática de IP flotantes a instancias ==="

# 1️⃣ Comprobar si existen instancias activas
echo "🔍 Buscando instancias activas..."
ACTIVE_SERVERS=$(openstack server list --status ACTIVE -f value -c Name)

if [[ -z "$ACTIVE_SERVERS" ]]; then
  echo "⚠️  No se encontraron instancias activas. Saliendo del bloque de asociación."
else
  echo "✅ Instancias detectadas:"
  echo "$ACTIVE_SERVERS"
  echo ""

  # 2️⃣ Para cada instancia, crear una IP flotante y asociarla
  for SERVER in $ACTIVE_SERVERS; do
    echo "🌍 Creando IP flotante para $SERVER ..."
    FLOATING_IP=$(openstack floating ip create -f value -c floating_ip_address red_externa)
    echo "   → IP flotante creada: $FLOATING_IP"

    echo "🔗 Asociando $FLOATING_IP a $SERVER ..."
    if openstack server add floating ip "$SERVER" "$FLOATING_IP"; then
      echo "   ✅ Asociación completada para $SERVER"
    else
      echo "   ⚠️  Error al asociar $FLOATING_IP a $SERVER"
    fi
...
# 3️⃣ Verificar gateway del router
ROUTER_NAME="router_privado"
echo "🔍 Verificando gateway del router $ROUTER_NAME..."
GATEWAY_INFO=$(openstack router show "$ROUTER_NAME" -f value -c external_gateway_info || echo "{}")

if [[ "$GATEWAY_INFO" == "{}" || -z "$GATEWAY_INFO" ]]; then
  echo "⚠️  El router $ROUTER_NAME no tiene gateway externo. Corrigiendo..."
  openstack router set "$ROUTER_NAME" --external-gateway red_externa
  echo "✅ Gateway establecido correctamente en $ROUTER_NAME."
else
  echo "✅ Router $ROUTER_NAME ya tiene gateway externo activo."
  echo "$GATEWAY_INFO"
fi

# 4️⃣ Mostrar resumen de IPs flotantes
echo ""
echo "📋 Estado final de las IPs flotantes:"
openstack floating ip list -f table

# 5️⃣ Test de conectividad opcional
echo ""
for SERVER in $ACTIVE_SERVERS; do
  FIP=$(openstack server show "$SERVER" -f value -c addresses | grep -oE '10\.0\.2\.[0-9]+')
  if [[ -n "$FIP" ]]; then
    echo "🏁 Probando ping hacia $SERVER ($FIP)..."
    ping -c 3 "$FIP" || echo "⚠️ Ping fallido para $SERVER ($FIP)"
  fi
done
